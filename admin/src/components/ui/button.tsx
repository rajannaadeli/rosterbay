 ...
 *
 * Every size carries a `coarse:` twin, one step up the ladder under a coarse
 * pointer. One step, not a jump to Apple's 44px: at 44 the controls read as an
 * oversized kiosk UI on a phone rather than as this product, and the density
 * the console is built around collapses. 36px with real spacing around it is
 * the size a small control wants to be here; the 44px idea is honoured by the
 * gaps and the row padding instead. A mouse never matches the media query, so
 * desktop metrics are unchanged.
...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-sm border border-transparent bg-clip-padding text-body font-medium whitespace-nowrap outline-none select-none transition-[background-color,border-color,color,box-shadow,transform] duration-[var(--duration-micro)] ease-[var(--ease-out)] focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/60 focus-visible:ring-offset-1 focus-visible:ring-offset-background active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-2 aria-invalid:ring-destructive/20 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      ...
      size: {
        default:
          "h-9 coarse:h-10 gap-1.5 px-3 has-data-[icon=inline-end]:pr-2.5 has-data-[icon=inline-start]:pl-2.5",
        xs: "h-6 coarse:h-8 gap-1 rounded-xs px-2.5 text-small has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2 [&_svg:not([class*='size-'])]:size-3",
        sm: "h-8 coarse:h-9 gap-1 px-3 text-small has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2",
        lg: "h-10 coarse:h-11 gap-1.5 px-4 has-data-[icon=inline-end]:pr-3 has-data-[icon=inline-start]:pl-3",
        icon: "size-9 coarse:size-10",
        "icon-xs": "size-6 coarse:size-8 rounded-xs [&_svg:not([class*='size-'])]:size-3",
        "icon-sm": "size-8 coarse:size-9",
        "icon-lg": "size-10 coarse:size-11",
      },
    },
    defaultVariants: {
      variant: "default",
