...
export function Segmented<T extends string | number>({
  ...
  return (
    <div
      ...
      aria-label={label}
      className={cn(
        'relative flex h-8 shrink-0 items-center rounded-sm border border-border-default bg-bg-base p-0.5 coarse:h-9',
        className,
      )}
    >
      <span
      ...
      {options.map((option) => {
        ...
        return (
          <button
            ...
            className={cn(
              'relative z-10 flex h-7 flex-1 items-center justify-center px-2.5 text-small font-medium coarse:h-8',
